...
Main proposed model: VMD_Optuna_TCN
...
Ablation (set run_ablation=True): adds "TCN" (raw signal) and "VMD_TCN" (VMD+TCN,
  no Optuna) alongside the full "VMD_Optuna_TCN".
...
CONFIG: Dict[str, Any] = {
    ...
    "models_to_run": ["VMD_Optuna_TCN", "GRU", "LSTM", "Transformer"],
    "reuse_existing_checkpoints": False,
    "save_long_prediction_file": True,

    # >>> NEW: ablation study. When True, two extra configurations are trained so
    # you can isolate the contribution of each component for the paper's ablation
    # table: "TCN" (TCN on the raw signal, no VMD/Optuna) and "VMD_TCN" (VMD+TCN
    # with a FIXED learning rate, no Optuna). The full model stays "VMD_Optuna_TCN".
    "run_ablation": False,

    # >>> NEW: fair-comparison switch. When True, GRU/LSTM/Transformer/TCN each get
    # their own Optuna learning-rate search (same budget as the proposed model),
    # so the proposed model's tuning is not an unfair advantage. Leave False to
    # reproduce the original fixed-lr baselines, but state your choice in the paper.
    ...
    "vmd": {
        ...
        "decompose_mode": "global",         # options: global, causal(see note)
    },
...
def build_model(model_name: str) -> nn.Module:
    ...
    # "TCN" is the raw-signal ablation: same TCN as the proposed model but trained
    # directly on the un-decomposed series (no VMD, no Optuna).
    if model_name in ("VMD_Optuna_TCN", "VMD_TCN", "TCN"):
        tcfg = CONFIG["tcn"]
...
def optuna_objective_tcn(trial, train_loader: DataLoader, val_loader: DataLoader) -> float:
    if optuna is None:
        raise ImportError(f"Optuna is required for VMD_Optuna_TCN. Install with: pip install optuna. Original error: {OPTUNA_IMPORT_ERROR}")
    device = torch.device(CONFIG["device"])
    lr = trial.suggest_float("lr", CONFIG["optuna"]["lr_low"], CONFIG["optuna"]["lr_high"], log=True)
    model = build_model("VMD_Optuna_TCN").to(device)
    optimizer = optim.Adam(model.parameters(), lr=lr)
    criterion = nn.MSELoss()
    try:
...
def tune_tcn_lr(train_loader: DataLoader, val_loader: DataLoader) -> float:
    if optuna is None:
        raise ImportError(f"Optuna is required for VMD_Optuna_TCN. Install with: pip install optuna. Original error: {OPTUNA_IMPORT_ERROR}")
    print(f"\n[{now_str()}] Optuna search for VMD_Optuna_TCN learning rate")
    study = optuna.create_study(direction="minimize")
    study.optimize(lambda trial: optuna_objective_tcn(trial, train_loader, val_loader),
                   n_trials=CONFIG["optuna"]["n_trials"])
    best_lr = float(study.best_params["lr"])
    trials_df = study.trials_dataframe()
    trials_path = os.path.join(DIRS["metrics"], "optuna_vmd_tcn_trials.csv")
    trials_df.to_csv(trials_path, index=False)
    print(f"  Best LR = {best_lr:.8f} | trials saved to {trials_path}")
    return best_lr


# >>> NEW: generic LR tuner so GRU/LSTM/Transformer/TCN can be Optuna-tuned with
# the SAME budget as the proposed model when CONFIG["tune_baselines"] is True.
...
def prepare_vmd_bundle(df_main: pd.DataFrame) -> Dict[str, Any]:
    if VMD is None:
        raise ImportError(f"vmdpy is required for VMD_Optuna_TCN. Install with: pip install vmdpy. Original error: {VMD_IMPORT_ERROR}")
    ...
    print(f"\n[{now_str()}] Preparing VMD modes for VMD_Optuna_TCN")
    for bid in target_buildings:
        col_name = find_building_column(df_main, bid)
        if col_name is None:
            print(f"  Skip {bid}: column not found")
...
def run_vmd_optuna_tcn(vmd_bundle: Dict[str, Any], df_all: pd.DataFrame,
                       df_main: pd.DataFrame, df_holdout: pd.DataFrame,
                       model_name: str = "VMD_Optuna_TCN", use_optuna: bool = True) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Dict[Tuple[str, str, str], pd.DataFrame]]:
    # Same rationale as the baselines: start the Optuna search and final training
    # from a fresh seed so this matches the standalone VMD script's RNG state.
    if CONFIG.get("reseed_per_model", True):
        set_seed(CONFIG["seed"])
    model_dir = os.path.join(DIRS["models"], model_name)
    model_path = os.path.join(model_dir, f"{model_name}_combined.pt")
    ...
    if CONFIG["reuse_existing_checkpoints"] and os.path.exists(model_path):
        ...
        if use_optuna:
            best_lr = tune_tcn_lr(vmd_bundle["train_loader"], vmd_bundle["val_loader"])
        else:
            # VMD_TCN ablation: VMD + TCN with a fixed learning rate (no Optuna).
        ...
        for row in history:
            row["Best_Optuna_LR"] = best_lr
        record_complexity(model_name, model, training_time, best_lr, CONFIG["seed"])
...
def run_experiment_for_seed(seed: int, df_all: pd.DataFrame, df_main: pd.DataFrame,
    ...
    # VMD-based models (proposed + optional no-Optuna ablation) share one bundle.
    need_vmd = ("VMD_Optuna_TCN" in CONFIG["models_to_run"]) or CONFIG.get("run_ablation", False)
    if need_vmd:
        vmd_bundle = prepare_vmd_bundle(df_main)
        if CONFIG.get("run_ablation", False):
            # VMD + TCN with fixed LR (no Optuna) -> isolates the Optuna contribution.
            metrics_rows, history_rows, predictions = run_vmd_optuna_tcn(
                vmd_bundle, df_all, df_main, df_holdout, model_name="VMD_TCN", use_optuna=False)
            seed_metrics.extend(metrics_rows)
            seed_history.extend(history_rows)
            seed_predictions.update(predictions)
        if "VMD_Optuna_TCN" in CONFIG["models_to_run"]:
            metrics_rows, history_rows, predictions = run_vmd_optuna_tcn(
                vmd_bundle, df_all, df_main, df_holdout, model_name="VMD_Optuna_TCN", use_optuna=True)
            seed_metrics.extend(metrics_rows)
            seed_history.extend(history_rows)
            seed_predictions.update(predictions)
...
def main() -> None:
    ...
    print("=" * 80)
    print("ONE-FILE EXPERIMENT: VMD_Optuna_TCN vs GRU, LSTM, Transformer")
    print("=" * 80)
