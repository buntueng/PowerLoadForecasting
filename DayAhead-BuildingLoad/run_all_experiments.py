...
CONFIG: Dict[str, Any] = {
    ...
    "vmd": {
        ...
        "init": 1,
        "tol": 1e-7,
        # Standalone fits each mode's StandardScaler on the FULL mode series
        # (scaler.fit_transform(u[i])). "full_series" reproduces that.
        # "train_only" is leakage-free but changes the training normalisation and
        # therefore the trained model, so results no longer match the standalone.
        "scale_fit_target": "full_series",  # options: full_series, train_only
        "scale_fit_unseen": "own_full",     # options: own_full
        # Ground truth used when scoring the reconstructed VMD forecast.
        #   "modes_sum" -> sum of the inverse-transformed mode targets
        #                  (== what the standalone script scores against).
        #   "original"  -> the true cleaned series (includes the VMD residual;
        #                  same target the GRU/LSTM/Transformer baselines use,
        #                  so it is the fairer cross-model comparison but yields
        #                  lower VMD scores than the standalone script).
        "eval_target": "modes_sum",         # options: modes_sum, original
    },
...
def main() -> None:
    ...
    all_predictions: Dict[Tuple[str, str, str], pd.DataFrame] = {}

    # Baseline models share the same raw-data split and scaler.
    baseline_names = [m for m in CONFIG["models_to_run"] if m in ["GRU", "LSTM", "Transformer"]]
    if baseline_names:
    ...
    if "VMD_Optuna_TCN" in CONFIG["models_to_run"]:
        ...
        metrics_rows, history_rows, predictions = run_vmd_optuna_tcn(vmd_bundle, df_all, df_main, df_holdout)
        all_metrics.extend(metrics_rows)
        all_history.extend(history_rows)
        all_predictions.update(predictions)
    ...
    history_path = os.path.join(DIRS["metrics"], "training_history.csv")
    history_df.to_csv(history_path, index=False)

    if CONFIG["save_long_prediction_file"]:
        long_predictions = pd.concat(all_predictions.values(), ignore_index=True) if all_predictions else pd.DataFrame()
    ...
    run_all_statistics(all_predictions, metrics_df)
